...
`findByTypeNameAll` can return more than one "UserRow"-named component where one wraps another (an outer `UserRow` renders an *unexecuted* `<UserRow .../>` element internally, which only becomes a second real "UserRow" render later once React actually calls it) — both ends up independently injecting an icon into what visually renders as the same single row, since the outer's patch runs and returns before the inner one has even executed, so it can't detect the inner's future icon. The fix: before injecting, check whether the current render still contains an un-executed nested `UserRow` element and skip if so — only the innermost (leaf) render actually adds the icon.
